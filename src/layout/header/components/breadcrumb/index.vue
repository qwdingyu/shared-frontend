<script setup lang="ts">
  ...
  const selectMenuItem = (menuItem: RouteRecordRaw | undefined) => {
    if (!menuItem) return
  ...
  const handleMenuSelect = (children: RouteRecordRaw[] | undefined, { key }: { key: string }) => {
    selectMenuItem(children?.find(item => item.name === key))
  }
</script>
    ...
    <template v-for="(routeItem, rotueIndex) in menus" :key="routeItem?.name">
      <a-breadcrumb-item>
        ...
        <template v-if="routeItem?.children?.length" #overlay>
          <a-menu
            :selected-keys="getSelectKeys(rotueIndex)"
            @select="handleMenuSelect(routeItem.children, $event)"
          >
