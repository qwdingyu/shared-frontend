...
export const useLayoutSettingStore = defineStore(
  ...
  () => {
    ...
    // 确保 menu.mode 始终使用默认值 'expand'，不被 localStorage 中的旧值覆盖
    const layoutSetting = reactive({
      ...defaultSetting,
      ...localLayoutSetting,
      menu: {
        ...defaultSetting.menu,
        ...(localLayoutSetting.menu?.locale ? { locale: localLayoutSetting.menu.locale } : {}),
        mode: defaultSetting.menu.mode, // 始终使用默认值 'expand'
      },
    })
  ...
  {
    persist: {
      paths: ['navTheme', 'colorPrimary', 'layout', 'contentWidth', 'fixedHeader', 'fixSiderbar', 'colorWeak', 'title', 'pwa', 'iconfontUrl', 'cacheTabs', 'size'],
    },
  } as any,
)
