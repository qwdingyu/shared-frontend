...
function sheetFromArrayOfArrays(data, XLSX: typeof import('xlsx')) {
  const { utils, SSF } = XLSX
  const ws = {}
  const range = {
    s: {
      c: 10000000,
...
export async function exportTableToExcel(id) {
  ...
  const wb = new Workbook(),
    ws = sheetFromArrayOfArrays(data, XLSX)
...
export async function exportJson2Excel({
