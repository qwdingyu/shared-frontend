...
// 从环境变量获取配置，支持不同项目使用不同配置
// web: VITE_BASE_API_URL = 'http://localhost:7001/api'
// web_mini: VITE_BASE_API_URL = 'http://localhost:5001/api'
const baseApiUrl = import.meta.env.VITE_BASE_API_URL || "";
const reportUrl = baseApiUrl
  ? `${baseApiUrl.replace(/\/api\/?$/, "")}/performanceMonitor`
  : "/performanceMonitor";

export default new PerformanceMonitor({
  reportUrl, // 性能上报地址，从环境变量动态获取
  appId: "performanceMonitor-1559318109525", // 项目ID
  appName: "performanceMonitor", // 项目名称
  env: import.meta.env.MODE || "dev", // 环境：从 Vite 模式获取
});
