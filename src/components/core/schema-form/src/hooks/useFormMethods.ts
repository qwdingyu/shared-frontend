...
export const useFormMethods = () => {
  ...
  function itemIsDateType(key: string) {
    const schemas = unref(formPropsRef).schemas as FormSchema[] | undefined;
    return schemas?.some((item: FormSchema) => {
  ...
  async function setFieldsValue(values: Recordable) {
    const schemas = unref(formSchemasRef) as FormSchema[];
    // @ts-ignore
        ...
        if (itemIsDateType(key)) {
          if (Array.isArray(value)) {
            ...
            set(
              formModel,
              key,
              value ? (_props?.valueFormat ? value : dayjs(value)) : null,
            );
          }
        } else {
  ...
  async function appendSchemaByField(
    ...
    const schemaList = cloneDeep(unref(formSchemasRef)) as FormSchema[];
  ...
  function getSchemaByField(field: string): FormSchema | undefined {
    return (unref(formSchemasRef) as FormSchema[]).find((schema) => field === schema.field);
  }
  ...
  const updateSchema = (data: Partial<FormSchema> | Partial<FormSchema>[]) => {
    ...
    (unref(formSchemasRef) as FormSchema[]).forEach((val) => {
  ...
  function handleFormValues(values: Recordable) {
    ...
    const { schemas } = formPropsRef.value;
    schemas?.forEach((x) =>
      fieldValue.push({ field: x.field, type: x.component }),
    );
