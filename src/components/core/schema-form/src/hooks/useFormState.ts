...
export const useFormState = ({ props, attrs }: useFormStateParams) => {
  ...
  const formSchemasRef = ref<UnwrapFormSchema[]>((formPropsRef.value.schemas || []) as unknown as UnwrapFormSchema[])
  watch(
    () => formPropsRef.value.schemas,
    schemas => {
      formSchemasRef.value = (schemas || []) as unknown as UnwrapFormSchema[]
    },
    { immediate: true },
  )
