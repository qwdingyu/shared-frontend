...
const getListResult = (res: unknown, listField: string): Recordable[] => {
  const configuredItems = get(res, listField)
  if (Array.isArray(configuredItems)) {
    return configuredItems
  }

  for (const field of ['list', 'data', 'records', 'items', 'rows']) {
    const items = get(res, field)
...
const getTotalResult = (res: unknown, totalField: string, fallbackTotal: number): number => {
  for (const field of [totalField, 'total', 'count', 'totalCount']) {
    const total = Number(get(res, field))
...
export const useTableMethods = () => {
  ...
  const fetchData = debounce(async (params: Recordable = {}) => {
    ...
    try {
      ...
      const safeResultItems: Recordable[] = isArrayResult ? res : getListResult(res, listField)
      const resultTotal: number = isArrayResult
        ? res.length
        : getTotalResult(res, totalField, safeResultItems.length)
      ...
      tableData.value = safeResultItems
      updatePagination({
        total: ~~(Number.isFinite(resultTotal) ? resultTotal : safeResultItems.length),
      })
