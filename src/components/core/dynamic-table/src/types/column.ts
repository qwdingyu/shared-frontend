...
export type TableColumn<T extends object = Recordable> = ColumnType<T> & {
  ...
  /** 多语言, 格式: \`t('common.add')\`，如果为空，默认以dataIndex作为key来翻译 */
  i18n?: string
  /** 不翻译多语言 (报表查询中使用) */
  ignoreI18n?: boolean
  children?: Array<any>
  ...
  /** 在 Table 中不展示此列 */
  hideInTable?: boolean
  /** 在 column-setting 中不展示此列 (解决report中存在的问题) */
  hideInColumnSet?: boolean
  ...
  /** 当前单元格是否默认开启编辑，仅 `editableType`为`cell` 或 `dynamicCell`时有效 */
