<template>
  <a-dropdown :class="[prefixCls]" :trigger="trigger" v-bind="$attrs">
    ...
    <template #overlay>
      <a-menu :class="[`${prefixCls}-menu`]" :selected-keys="selectedKeys">
        <template v-for="item in dropMenuList" :key="`${item.event}`">
          <a-menu-item
            v-if="item?.vIf == null || item.vIf"
          ...
          >
            ...
            >
              <template v-if="item.popConfirm.icon" #icon>
                <Icon type="icon-font" :icon="item.popConfirm.icon" />
              </template>
              <div class="dropdown-event-area">
                <Icon v-if="item.icon" type="icon-font" :icon="item.icon" />
...
<script lang="ts" setup>
  ...
  const prefixCls = 'mom-basic-dropdown'
